...
@bot.tree.command(
    name="clear_add", description="Добавить пользователя в список разрешенных"
)
async def clear_add(interaction: discord.Interaction, user: discord.User):
    if interaction.user.id == interaction.guild.owner_id:
        ...
        if os.path.exists(USER_FILE):
            ...
            if str(user_id) in allowed_users:
                await interaction.response.send_message(
                    f"Пользователь {user.mention} уже добавлен в список разрешенных.",
                    ephemeral=True,
                )
                return
        ...
        await interaction.response.send_message(
            f"Пользователь {user.mention} добавлен в список разрешенных.",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
            "Только владелец сервера может добавлять пользователей.",
            ephemeral=True,
        )
...
@bot.tree.command(
    name="clear_remove",
    description="Удалить пользователя из списка разрешенных",
)
        ...
        if os.path.exists(USER_FILE):
            ...
            if str(user_id) in allowed_users:
                ...
                with open(USER_FILE, "w") as file:
                    file.write("\n".join(allowed_users))
                await interaction.response.send_message(
                    f"Пользователь {user.mention} удален из списка разрешенных.",
                    ephemeral=True,
                )
            else:
                await interaction.response.send_message(
                    f"Пользователь {user.mention} не найден в списке.",
                    ephemeral=True,
                )
        else:
            await interaction.response.send_message(
                "Файл с разрешенными пользователями не найден.", ephemeral=True
            )
    else:
        await interaction.response.send_message(
            "Только владелец сервера может удалять пользователей.",
            ephemeral=True,
        )
...
class ConfirmClearView(discord.ui.View):
    ...
    async def confirm(
        ...
        if interaction.user.id != self.interaction.user.id:
            await interaction.response.send_message(
                "Вы не инициировали эту команду.", ephemeral=True
            )
            return
        await interaction.response.defer()

        deleted_messages = await self.interaction.channel.purge(
            limit=self.amount
        )
...
async def clear(interaction: discord.Interaction, amount: int = 10000):
    if amount <= 0:
        await interaction.response.send_message(
            "Количество сообщений для удаления должно быть больше 0.",
            ephemeral=True,
        )
        return

    if interaction.user.id == interaction.guild.owner_id or is_user_allowed(
        interaction.user.id
    ):
        if amount == 10000:
        ...
        if count == 0:
            await interaction.followup.send(
                "В данном канале нет сообщений.", ephemeral=True
            )
        else:
            declension = get_message_declension(count)
            await interaction.followup.send(
                f"Последние {count} {declension} в данном канале удалены.",
                ephemeral=True,
            )
    else:
        await interaction.response.send_message(
...
@bot.tree.command(
    name="clear_show", description="Показать добавленных пользователей"
)
async def clear_show(interaction: discord.Interaction):
    if os.path.exists(USER_FILE):
        ...
        if allowed_users:
            ...
            await interaction.response.send_message(
                "Список разрешенных пользователей пуст.", ephemeral=True
            )
    else:
        await interaction.response.send_message(
            "Файл с разрешенными пользователями не найден.", ephemeral=True
...
async def lock(
    ...
    if user.guild_permissions.administrator:
        await interaction.followup.send(
            "❌ Нельзя ограничить администратора.", ephemeral=True
        )
        return

    if user.top_role >= interaction.guild.me.top_role:
        await interaction.followup.send(
            "❌ У пользователя роль выше или равна роли бота. Ограничение невозможно.",
            ephemeral=True,
        )
        return

    if (amount is None and unit is not None) or (
        amount is not None and unit is None
    ):
        await interaction.followup.send(
    ...
    if amount == 0:
        await interaction.followup.send(
            "⚠️ Значение времени не может быть равно 0.", ephemeral=True
        )
        return
    ...
    if scope == "channel":
        ...
        if not chat_banned_role:
            await interaction.followup.send(
                "Не найдена роль chat banned.", ephemeral=True
            )
            return
        ...
        if amount and unit:
            if unit.value not in UNITS:
                await interaction.followup.send(
                    "Неверно указана единица времени.", ephemeral=True
                )
                return
            ...
            if seconds > MAX_TIMEOUT_SECONDS:
                await interaction.followup.send(
                    "Максимальное время таймаута — 28 дней.", ephemeral=True
                )
                return
        ...
        try:
            ...
            await interaction.followup.send(
                "❌ Нет прав ограничить этого пользователя.", ephemeral=True
            )
            return
...
@bot.tree.command(
    name="unlock",
    description="Снять ограничение на отправку сообщений у пользователя",
)
async def unlock(
    interaction: discord.Interaction,
    user: discord.Member,
    ...
    if user.guild_permissions.administrator:
        await interaction.followup.send(
            "❌ У администратора нет ограничений.", ephemeral=True
        )
        return

    if user.top_role >= interaction.guild.me.top_role:
        await interaction.followup.send(
            "❌ У пользователя роль выше или равна роли бота. Снятие невозможно.",
            ephemeral=True,
        )
        return
    ...
    if scope.value == "channel":
        ...
        if not chat_banned_role:
            await interaction.followup.send(
                "❌ Роль chat banned не найдена.", ephemeral=True
            )
            return
        ...
        if chat_banned_role in user.roles:
            ...
            await interaction.followup.send(
                f"{user.mention} не был заблокирован в канале.", ephemeral=True
            )
        ...
        try:
            ...
            await interaction.followup.send(
                "❌ Нет прав разблокировать пользователя.", ephemeral=True
            )
